...
def _goal_tick_action_pack(
    ...
    system = "\n".join(
        [
            ...
            '- {"type":"mcp.tool","input":{"server":"gym-telegram|gym-weather|gym-googlecalendar|gym-sendgrid|gym-weight","tool":"...","args":{...}}}',
            "",
            "Rules:",
            ...
            "- The user payload may include `[context]` with `calendarNext21Days` lines — each line starts with `eventId=<google_id> |` when known. "
            "`goalPlan` lists planned steps that may already include `eventId=...` for linked Calendar rows.",
            "- Calendar hygiene (CRITICAL):",
            "  * To **remove duplicates**: find events with the same or nearly the same title and overlapping start times; **keep one**; emit `googlecalendar_delete_event` with each **duplicate's** `eventId` (from context). Never try to dedupe by creating more events.",
            "  * To **change times / reschedule / daytime preference**: use `googlecalendar_update_event` with `eventId`, `startISO`, `endISO` (and optional `summary`). Do **not** create a new event for a time change.",
            ...
            "- Avoid **redundant** creates only: if the calendar observation already shows the same workout at the same time, skip another create; otherwise you **must** create. ",
            "- **/goal tick** sync: include `googlecalendar_create_event` for each `goal.plan` step that has **no** `eventId` and is not already represented in calendar context. ",
            ...
            "- If you create a **new** plan step with whenISO/durationMinutes and it has **no** eventId yet, also emit a matching `googlecalendar_create_event`. "
            "If the step should replace an existing calendar row, use `googlecalendar_update_event` with that row's eventId instead.",
